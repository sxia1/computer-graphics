...
void first_pass() {
  ...
  if(num != 0 && strlen(name) == 0){
    strcpy(name, "ANIM");
    printf("WARNING: FRAMES set but no BASENAME found, set to default: ANIM");
  }
}
...
struct vary_node ** second_pass() {
  ...
  for (int i = 0; i < lastop; i++) {
	...
	double incr = (op[i].op.vary.end_val - op[i].op.vary.start_val)/(op[i].op.vary.end_frame-op[i].op.vary.start_frame +1);
	printf("%lf", incr);
	double val = op[i].op.vary.start_val;
	for(int j = op[i].op.vary.start_frame; j <= op[i].op.vary.end_frame; j ++){
	  struct vary_node *new_node = malloc(sizeof(struct vary_node));
	  strcpy(new_node->name, op[i].op.basename.p->name);
	  new_node->value = val;
	  new_node->next = NULL;
	  curr = knobs[j];
	  if(!curr){
	    ...
	    //if the knob was already defined
	    if(!strcmp(curr->name, op[i].op.save_knobs.p->name)){
	      curr->value = new_node->value;
	    }
	    else{
	      curr->next = new_node;
	    }
...
void my_main() {
  ...
  for(int k = 0; k < num_frames; k ++){
    ...
    //naming the frames
    sprintf(frame_name, "anim/%s_%03d.png", name, k);
