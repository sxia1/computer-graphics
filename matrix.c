...
struct matrix * make_bezier() {
  struct matrix *bez;
  bez = new_matrix(4, 4);
  bez->m[0][0] = -1; bez->m[0][1] = 3; bez->m[0][2] = -3; bez->m[0][3] = 1;
  bez->m[1][0] = 3; bez->m[1][1] = -6; bez->m[1][2] = 3;
  bez->m[2][0] = -3; bez->m[2][1] = 3;
  bez->m[3][0] = 1;
  return bez;
}
...
struct matrix * make_hermite() {
  struct matrix *herm;
  herm = new_matrix(4, 4);
  herm->m[0][0] = 2; herm->m[0][1] = -2; herm->m[0][2] = 1; herm->m[0][3] = 1;
  herm->m[1][0] = -3; herm->m[1][1] = 3; herm->m[1][2] = -2; herm->m[1][3] = -1;
  herm->m[2][2] = 1;
  herm->m[3][0] = 1;
  return herm;
}
...
struct matrix * generate_curve_coefs( double p1, double p2,
  ...
  if(type == HERMITE){
    matrix_mult(make_hermite(), vals);
  }
  else if(type == BEZIER){
    matrix_mult(make_bezier(), vals);
  }
  return vals;
}
